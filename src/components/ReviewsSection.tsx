  ...
  <motion.div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-10"
    ...
    >
      ...
      <div className="relative h-28 overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: `linear-gradient(hsl(var(--${review.color})) 1px, transparent 1px), linear-gradient(90deg, hsl(var(--${review.color})) 1px, transparent 1px)`, backgroundSize: "32px 32px" }} />
        <div className="absolute inset-0 flex items-end p-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full flex items-center justify-center font-ui text-xs font-bold shrink-0"
              style={{ background: `hsl(var(--${review.color}) / 0.2)`, color: `hsl(var(--${review.color}))`, boxShadow: `0 0 16px hsl(var(--${review.color}) / 0.3)` }}>
              {review.avatar}
            </div>
            <div>
...
const ReviewCard = ({ review, index, onOpen }: { review: Review; index: number; onOpen: (r: Review) => void }) => (
  <motion.div
    ...
    whileHover={{ translateZ: 14, rotateX: -3, transition: { duration: 0.3 } }}
  ...
  >
    <div className="absolute top-0 left-0 right-0 h-0.5" style={{ background: `linear-gradient(90deg, hsl(var(--${review.color})), transparent)` }} />
    ...
    <p className="font-body text-sm text-muted-foreground leading-[1.75] font-light flex-1 line-clamp-3">
      "{review.text}"
    </p>
    <div className="flex items-center gap-2.5 pt-2.5 border-t border-border/50">
      <div className="w-8 h-8 rounded-full flex items-center justify-center font-ui text-[10px] font-bold shrink-0"
        style={{ background: `hsl(var(--${review.color}) / 0.15)`, color: `hsl(var(--${review.color}))`, boxShadow: `0 0 10px hsl(var(--${review.color}) / 0.2)` }}>
        {review.avatar}
      </div>
    ...
    <motion.button
      ...
      whileTap={{ scale: 0.97 }}
      className="w-full mt-1 py-2 rounded-xl border font-ui text-[11px] font-semibold tracking-wide transition-all duration-200 flex items-center justify-center gap-1.5"
      style={{
        borderColor: `hsl(var(--${review.color}) / 0.3)`,
        background: `hsl(var(--${review.color}) / 0.06)`,
        color: `hsl(var(--${review.color}))`,
      }}
    >
      Read Full Review
